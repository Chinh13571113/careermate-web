...
export default function RecruiterManagementPage() {
  ...
  const [filterStatus, setFilterStatus] = useState<
    "all" | "PENDING" | "APPROVED" | "ACTIVE" | "REJECTED" | "BANNED"
  >("all");
  ...
  const fetchRecruiters = async () => {
    try {
      ...
      if (response.code === 200 && response.result?.content) {
        ...
        console.log(
          "✅ [Recruiter Management] Fetched recruiters:",
      ...
      setError(
        error.message || "Error loading recruiters. Please try again later."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleBanConfirm = async () => {
    ...
    try {
      ...
      const response = await banRecruiter(
        selectedRecruiter.accountId,
        banReason
      );
      console.log("✅ [Ban] Response:", response);
      console.log(
        `📝 [Ban] Current status "${selectedRecruiter.accountStatus}" will be restored after unban`
      );

      alert(
        "Recruiter banned successfully. Their current status will be restored if unbanned later."
      );
  ...
  const pendingCount = recruiters.filter(
    (r) => r.accountStatus?.toUpperCase() === "PENDING"
  ).length;
  ...
  const rejectedCount = recruiters.filter(
    (r) => r.accountStatus?.toUpperCase() === "REJECTED"
  ).length;

  const bannedCount = recruiters.filter(
    (r) => r.accountStatus?.toUpperCase() === "BANNED"
  ).length;
        ...
        <div className="mt-6 flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              className={
                filterStatus === "BANNED"
                  ? "rounded-lg bg-gray-700 hover:bg-gray-800 text-white"
                  : "rounded-lg"
              }
            >
        ...
        <>
                ...
                {filteredRecruiters.map((recruiter) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge
                          className={`${verificationBadge.color} px-3 py-1 font-medium flex items-center w-fit`}
                        >
                      ...
                      <TableCell className="text-center">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {recruiter.accountStatus?.toUpperCase() !==
                            "BANNED" && (
                            <Button
                              variant="ghost"
                              size="sm"
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mt-6">
            <div className="text-sm text-gray-600 text-center">
              Showing{" "}
              <span className="font-medium">{filteredRecruiters.length}</span>{" "}
              of <span className="font-medium">{recruiters.length}</span>{" "}
              recruiters
            </div>
          </div>
        </>
      ...
      <Dialog open={isBanDialogOpen} onOpenChange={setIsBanDialogOpen}>
        <DialogContent className="rounded-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              This action will ban the recruiter and prevent them from accessing
              the system.
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm text-amber-800">
                    ...
                    <ul className="list-disc list-inside space-y-1">
                      <li>
                        The recruiter will be immediately banned from accessing
                        their account
                      </li>
                      <li>
                        Current status "
                        ...
                        " will be saved and restored after unban
                      </li>
                      <li>
                        All their posted jobs may be hidden until unbanned
                      </li>
                    </ul>
                  </div>
                </div>
